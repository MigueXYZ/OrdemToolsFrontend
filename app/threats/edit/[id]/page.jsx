...
import styles from '../../../manage/AddThreatForm.module.css';
...
export default function EditThreatPage() {
  const { user } = useContext(AuthContext);
  const router = useRouter();
  const params = useParams(); // Busca o [id] da URL
  ...
  // Carregar os dados iniciais da Ameaça
  useEffect(() => {
    const fetchThreat = async () => {
      try {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/threats/${params.id}`);
        const data = res.data;
        
        // Formatação dos dados para o State do formulário
        setFormData({
          name: data.name || '',
          vd: data.vd || '',
          ...
          type: data.type || '',
          size: data.size || '',
          elements: Array.isArray(data.elements) ? data.elements.join(', ') : '',
          defense: data.defense || '',
          hpTotal: data.hp?.total || '',
          hpBloodied: data.hp?.bloodied || '',
          movement: data.movement || '',
          resistances: Array.isArray(data.resistances) ? data.resistances.join(', ') : '',
          vulnerabilities: Array.isArray(data.vulnerabilities) ? data.vulnerabilities.join(', ') : '',
          attributes: data.attributes || { agi: 0, for: 0, int: 0, pre: 0, vig: 0 },
          senses: data.senses || { perception: '', initiative: '', notes: '' },
          savingThrows: data.savingThrows || { fortitude: '', reflexes: '', will: '' },
          skills: data.skills || [],
          passives: data.passives || [],
          actions: data.actions || [],
          enigmaOfFear: data.enigmaOfFear || { hasEnigma: false, description: '', mechanics: '' },
          disturbingPresence: data.disturbingPresence || { hasDisturbingPresence: false, dt: '', damage: '', immunityNex: '' },
          book: data.book || ''
        });
      } catch (err) {
        console.error(err);
        setMessage({ type: 'error', text: 'Não foi possível carregar os dados desta ameaça.' });
      } finally {
        setFetching(false);
      }
    };

    if (params.id) fetchThreat();
  }, [params.id]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.patch(
        `${process.env.NEXT_PUBLIC_API_URL}/threats/${params.id}`, 
        payload,
        {
          headers: {
      ...
      alert('Ameaça atualizada na base de dados com sucesso!');
      window.location.assign('/browse?tab=threats'); // Envia de volta para a lista
  ...
  if (fetching) return <div style={{ color: 'white', padding: '2rem', textAlign: 'center' }}>A carregar dados confidenciais...</div>;

  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '2rem' }}>
      <button onClick={() => router.back()} style={{ background: 'transparent', color: 'var(--text-accent)', border: 'none', cursor: 'pointer', marginBottom: '1rem', fontWeight: 'bold' }}>
        &larr; Cancelar Edição
      </button>
      
      <form onSubmit={handleSubmit} className={styles.aeroForm}>
        <div className={styles.formHeader}>
          <h3 className={styles.formTitle}>Editar Ameaça: {formData.name}</h3>
        </div>
        ...
        <div className={styles.formContent}>
          ...
          <button type="submit" disabled={loading} className={styles.aeroButton} style={{ marginTop: '2rem', padding: '1rem' }}>
            {loading ? 'A Gravar...' : 'Atualizar Ameaça'}
          </button>
        </div>
      </form>
    </div>
